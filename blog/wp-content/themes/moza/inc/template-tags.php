...
if ( ! function_exists( 'moza_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time.
	 */
	function moza_posted_on() {
...
if ( ! function_exists( 'moza_posted_by' ) ) :
	/**
	 * Prints HTML with meta information for the current author.
	 */
	function moza_posted_by() {
		$byline = sprintf(
			/* translators: %s: post author. */
			esc_html( ' %s', 'post author', 'moza' ),
...
if ( ! function_exists( 'moza_category' ) ) :
	function moza_category() {
		if ( 'post' === get_post_type() ) {
			/* translators: used between list items, there is a space after the comma */
			$categories_list = get_the_category_list( esc_html__( ', ', 'moza' ) );
			if ( $categories_list ) {
				/* translators: 1: list of categories. */
				printf( '<span class="cat-links">' . esc_html( '%1$s', 'moza' ) . '</span>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
	}
endif;
